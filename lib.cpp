...
static inline bool strict_search(const std::string& source_term,
                                 const std::string& search_term) {
    return source_term == search_term;
}

static inline bool insensitive_search(const std::string& source_term,
                                      const std::string& search_term) {
...
static inline bool fuzzy_search(const std::string& source_term,
                                const std::string& search_term) {
    const std::string source = to_lowercase(source_term);
    const std::string test = to_lowercase(search_term);
...
static inline bool contains_alternative(
    const std::string& term,
    std::string* found_alternative = nullptr) {
...
static inline std::string random_scheiss(const TokenAnalysis& token) {
    const auto& alternatives =
        (token.type == Artikel && token.declination_type != Declination_Unknown)
            ? (token.declination_type == MixedFlexion ? alternatives_mixed_flex
                                                      : alternatives_known)
            : alternatives_unknown;

    for (int index = default_alternative + 1; index < alternatives.size();
         index++) {
        const auto& alternative = alternatives[index];
        if ((alternative.genus & token.genus) &&
            (alternative.casus & token.casus)) {
...
static inline std::vector<std::string> split_string(
...
static inline std::vector<std::string>::const_iterator find_duplicates(
    const std::vector<std::string>& clearable,
...
static inline std::vector<Spe>::const_iterator find_duplicate_spes(
    const std::vector<Spe>& clearable,
...
static const std::array<std::string, 3> articles_nominativ_singular_known = {
    "der", "die", "das"};
static const std::array<std::string, 3> articles_genitiv_singular_known = {
    "des", "der", "des"};
static const std::array<std::string, 3> articles_dativ_singular_known = {
    "dem", "der", "dem"};
...
static const std::array<std::string, 3> articles_genitiv_singular_mixedflex = {
    "eines", "einer", "eines"};
static const std::array<std::string, 3> articles_dativ_singular_mixedflex = {
    "einem", "einer", "einem"};
...
static inline std::vector<Genus> potential_genuses_for_article(
    const std::string& term) {
...
static inline Case casus_for_article(const std::string& article,
                                     const Genus genus) {
    if ((article == articles_nominativ_singular_known[0] && (genus & Male)) ||
        ...
        return Nominativ;
             ...
             (article == articles_genitiv_singular_known[1] &&
              (genus & Female)) ||
        ...
        return Dativ;
    else if ((article == articles_akkusativ_singular_known[0] &&
              (genus & Male)) ||
             (article == articles_akkusativ_singular_known[1] &&
              (genus & Female)) ||
             (article == articles_akkusativ_singular_known[2] &&
              (genus & Neutrum)))
        return Akkusativ;
...
static inline DeclinationType declination_type_for_article(
    const std::string& term,
    const Genus genus) {
...
#define TRY_RETURN_TYPE(arr, term, genus, type)    \
    ...
    // With article?
    TRY_RETURN_TYPE(articles_nominativ_singular_known, term, genus,
                    FlexionWithArticle);
    TRY_RETURN_TYPE(articles_genitiv_singular_known, term, genus,
                    FlexionWithArticle);
    TRY_RETURN_TYPE(articles_dativ_singular_known, term, genus,
                    FlexionWithArticle);
    TRY_RETURN_TYPE(articles_akkusativ_singular_known, term, genus,
                    FlexionWithArticle);

    // Mixed flexion?
    TRY_RETURN_TYPE(articles_nominativ_singular_mixedflex, term, genus,
                    MixedFlexion);
    TRY_RETURN_TYPE(articles_genitiv_singular_mixedflex, term, genus,
                    MixedFlexion);
    TRY_RETURN_TYPE(articles_dativ_singular_mixedflex, term, genus, MixedFlexion);
    TRY_RETURN_TYPE(articles_akkusativ_singular_mixedflex, term, genus,
                    MixedFlexion);
...
static inline Case preposition_search(const std::string& term) {
    ...
    static const std::array<std::string, 16> dative_prepositions = {
                                                                    "ab",      "aus",     "außer",    "bei",   "gegenüber", "mit",
                                                                    "nach",    "nächst",  "nebst",    "seit",  "von",       "zu",
                                                                    "zufolge", "zuliebe", "zunächst", "zuwide"};
...
static inline bool adverb_search(const std::string& term) {
    static const std::array<std::string, 228> adverbs = {"abends",
                                                         "aber",
                                                         "alle",
                                                         "allein",
                                                         "allerdings",
...
static inline bool subjunction_search(const std::string& term) {
    static const std::array<std::string, 28> subjunctions = {
                                                             ...
                                                             "ehe",     "falls",   "indem", "nachdem",   "ob",     "obgleich",
                                                             "obschon", "obwohl",  "seit",  "seitdem",   "sobald", "sodass",
                                                             "sofern",  "solange", "sooft", "soweit",    "soviel", "während",
                                                             "weil",    "wenn",    "wie",   "wohingegen"};
...
static inline bool nomen_check(const std::string& word) {
    return word.length() >= 2 && word[0] == std::toupper(word[0]) &&
           word[1] == std::tolower(word[1]);
}

static inline bool adjective_check(const TokenAnalysis* before,
...
static inline bool pronomen_check(const std::string& word) {
    static const std::vector<std::string> pronomen = {
        "ich", "du", "er", "sie", "es", "wir", "ihr", "sie", // Personalpronomen
        "sich", // Reflexivpronomen
        "mein", "dein", "sein", "unser", "euer", "ihr", // Possessivpronomen
        "der", "dieser", "jener", "derjenige", "derselbe", // Demonstrativpronomen
        "man", "jemand", "niemand", "jeder", "alle", "etwas", "nichts", // Indefinitpronomen
        "wer", "was", "welcher", // Interrogativpronomen, TODO: "was für ein"
        "der", "welcher", "wer", "was" // Relativpronomen
    };

    for (const auto& pronom : pronomen) {
        if (insensitive_search(word, pronom)) {
            return true;
        }
    }
...
static inline std::vector<Spe> article_verscheissern(
    ...
    // Opinionated workaround: with two articles in a row, prefer to verscheisser
...
static inline std::vector<Spe> nomen_verscheissern(
    ...
    // TODO: Maybe do this without ruling out completely valid grammatical
    // circumstances.
    if (token.before_token && token.before_token->before_token &&
        (token.before_token->type == Artikel) &&
        (token.before_token->before_token->type == Artikel)) {
        ret.push_back({token.word, true});
        return ret;
    }
...
static inline std::vector<Spe> adjective_verscheissern(
    const std::vector<TokenAnalysis>& analysis,
    const TokenAnalysis& token) {
...
static inline std::string strip_punctuation(const std::string& word,
    ...
    for (const auto& c : word) {
        if (std::find(punctuations.begin(), punctuations.end(), c) !=
...
static inline std::vector<TokenAnalysis>::iterator find_next_nomen(
    std::vector<TokenAnalysis>::iterator begin,
...
static inline Genus find_article_genus_for_casus(
    const std::string& article,
    const std::vector<Genus>& genuses,
    const Case casus) {
    for (const auto& genus : genuses) {
        switch (casus) {
        case Nominativ: {
            const auto found =
                articles_nominativ_singular_known[article_index_for_genus(genus)];
            if (insensitive_search(found, article))
                return genus;
        }
        case Genitiv: {
            const auto found =
                articles_genitiv_singular_known[article_index_for_genus(genus)];
            if (insensitive_search(found, article))
                return genus;
        }
        case Dativ: {
            const auto found =
                articles_dativ_singular_known[article_index_for_genus(genus)];
            if (insensitive_search(found, article))
                return genus;
        }
        case Akkusativ: {
            const auto found =
                articles_akkusativ_singular_known[article_index_for_genus(genus)];
            if (insensitive_search(found, article))
                return genus;
        }
        default:
...
static inline void build_relations(std::vector<TokenAnalysis>& analysis) {
    for (auto it = analysis.begin(); it != analysis.end(); it++) {
        ...
        // Example: "Des Vodkas reinster Vergleichspunkt ist ->die der Kartoffel
        // nachgesagte Verarbeitung<-."
        if (peek_forward_token && token.type == Artikel &&
            (*peek_forward_token).type == Artikel) {
        ...
        else if (peek_forward_token && token.type == Artikel &&
                 (*peek_forward_token).type == Nomen) {
...
std::string verscheissern(const std::vector<std::string>& input,
    ...
    for (auto it = analysis.cbegin(); it != analysis.cend(); it++) {
        ...
        if ((flags & Replacements) && replacement_search(token.word)) {
            ...
            spes =
                article_verscheissern(look_backward_token, token, peek_forward_token);
        } else if ((flags & ScheissFlags::BeforeNomen) && token.type == Nomen &&
                   (look_backward_token &&
                    token.dictating_token == look_backward_token) &&
                   /* TODO: Remove token_type check once smarter */
                   (token.token_type != SentenceBeginning)) {
            spes = nomen_verscheissern(analysis, look_backward_token, token,
                                       peek_forward_token);
        } else if ((flags & ScheissFlags::BeforeAdjectives) &&
                   token.type == Adjektiv &&
                   /* TODO: Remove token_type check once smarter */
                   (token.token_type != SentenceBeginning)) {
